...
export default function TrustMintModal() {
    ...
    const { state, handleTrustAction, setLoading } = useTrustMintContext();
    const [defaultMint, setDefaultMint] = useState("");

    const handleAutoSwap = () => {
        if (!state.tokenInfo) {
            return openPromptAutoClose({
                msg: "tokenInfo is undefined or user has no default mint",
            });
        }
        handleTrustAction("swap");
        nav.navigate("processing", {
            mint: {
                mintUrl: state.tokenInfo.mints[0],
                customName: "",
            },
    ...
    const handleTrust = () => {
        setLoading(true);
        handleTrustAction("trust");
    };

    const handleCancel = () => {
        handleTrustAction("cancel");
    };
        ...
        >
            ...
            {state.tokenInfo && (
                <Text style={[styles.mintPrompt, { color: color.TEXT }]}>
                    {formatSatStr(state.tokenInfo.value)} {t("from")}:
                </Text>
            )}
            {/* Show in which mint(s) the tokens are */}
            <View style={styles.tokenMintsView}>
                {state.tokenInfo?.mints.map((m) => (
                    <Text
            ...
            <TouchableOpacity
                ...
                style={[
                    styles.container,
                    { opacity: defaultMint.length === 0 ? 0.4 : 1 },
                ]}
            ...
            <TouchableOpacity style={styles.container} onPress={handleTrust}>
                <View style={styles.iconContainer}>
                    {state.loading ? (
                        <View>
