  ...
  useEffect(() => {
    if (!permission?.granted) {
      ...
      if (isIOS) {
        ...
        setTimeout(() => {
          console.log("🎯 [QR SCAN] iOS: Camera ready state set after delay");
          setCameraReady(true);
        }, CAMERA_READY_DELAY / 2);
      } else {
  ...
  const handleCashuToken = useCallback(
    async (data: string) => {
      ...
      if (
        ...
        try {
          const action = await trustMintRef.current?.open(info.decoded);
          if (action === "trust") {
  ...
  const handleBarCodeScanned = useCallback(
    ({ type, data }: { type: string; data: string }) => {
      ...
      try {
        ...
        console.log("✅ [QR SCAN] Valid QR code scanned, processing...");
    ...
    <View style={[styles.container, { backgroundColor: color.BACKGROUND }]}>
      ...
      <View style={styles.cameraSection}>
        {!permission?.granted ? (
          ...
          <View style={[styles.cameraLoading, { paddingTop: insets.top + 80 }]}>
            <Loading size={40} />
            <Text style={[styles.cameraLoadingText, { color: color.TEXT }]}>
              {t("loadingCamera") || "Loading camera..."}
            </Text>
            {isIOS && (
              <Text
                style={[styles.cameraHintText, { color: color.TEXT_SECONDARY }]}
              >
                Initializing camera...
              </Text>
            )}
          ...
          >
            <View style={styles.cameraWrapper}>
              <CameraView
                ...
                onMountError={(error) => {
                  console.error("❌ [QR SCAN] Camera mount error:", error);
                  openPromptAutoClose({
                    msg:
                      t("cameraError") ||
              ...
              {scanned && !processing && (
                <View style={styles.scanAgain}>
                  <TouchableOpacity onPress={resetScan}>
                    <Text style={styles.scanAgainTxt}>{t("scanAgain")}</Text>
                  </TouchableOpacity>
                </View>
              )}
              {processing && (
