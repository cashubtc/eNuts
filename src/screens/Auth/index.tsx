...
export default function AuthPage({ navigation, route }: TAuthPageProps) {
  ...
  const handlePinMismatch = async () => {
    ...
    const attemptState = {
      ...
      lockedCount: maxMismatchCount
        ? increasedLockedCount
        : attempts.lockedCount,
    ...
    if (!isConfirm) {
      await store.setObj(STORE_KEYS.lock, {
        ...attemptState,
          ...
          {attempts.locked && !isConfirm ? (
            ...
            <View style={styles.content}>
              ...
              <View style={styles.pinpadWrap}>
                ...
                {(((shouldRemove || shouldEdit) && auth.length > 0) ||
                  (shouldEdit && !auth.length)) && (
                  <TxtButton
                    txt={t("cancel")}
                    onPress={() => {
                      resetStates();
