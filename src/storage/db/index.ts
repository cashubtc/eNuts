...
export async function addMint(mintUrl: string, id = ''): Promise<boolean> {
	...
	if (!id) {
		id = await getCurrentKeySetId(mintUrl)
	}
...
export async function addAllMintIds() {
	...
	for (const mint of mints) {
		...
		for (const id of ids) {
			if (mints.some((x) => x.id === id && x.mintUrl === mint.mintUrl)) {
				continue
			}
			// eslint-disable-next-line no-await-in-loop
...
export async function delInvoice(hash: string) {
	const result = await db.execTx<IInvoice>('Delete from invoices Where hash = ?', [hash])
	l('[delInvoice]', result, { hash })
	return result.rowsAffected === 1
}
export async function getInvoice(hash: string) {
	const result = await db.execSelect<IInvoice>('SELECT * from invoices Where hash = ?', [hash])
...
export async function addContact(c: IContact) {
	const result = await db.execInsert('INSERT INTO contacts (name, ln, isOwner) VALUES (?, ?, ?)', [
		c.name,
		c.ln,
		c?.isOwner ? 1 : 0,
	])
...
export async function editContact(c: Required<IContact>) {
	const result = await db.execTx('UPDATE contacts SET name = ? , ln = ? WHERE id = ?', [c.name, c.ln, c.id])
	l('[editContact]', result, c)
	return result.rowsAffected === 1
}
export async function delContact(id: number) {
	const result = await db.execTx('Delete from contacts Where id = ?', [id])
