...
export class KeysetRepository {
    ...
    async saveKeyset(keyset: Omit<Keyset, "updatedAt">): Promise<boolean> {
        const sql = `
            INSERT OR REPLACE INTO keysets (mint_url, id, keypairs, active, fee_ppk, updated_at) 
    ...
    async getKeyset(id: string): Promise<Keyset | null> {
        const sql = "SELECT * FROM keysets WHERE id = ?";
    ...
    async getKeysetsByMintUrl(mintUrl: string): Promise<Keyset[]> {
        const sql = "SELECT * FROM keysets WHERE mint_url = ? ORDER BY id";
    ...
    async getAllKeysets(): Promise<Keyset[]> {
        const sql = "SELECT * FROM keysets ORDER BY mint_url, id";
    ...
    async updateKeyset(id: string, updates: Partial<Keyset>): Promise<boolean> {
        ...
        const sql = `UPDATE keysets SET ${setClause}, updated_at = cast(strftime('%s','now') as INTEGER) WHERE id = ?`;
        const params = [
            ...fields
    ...
    async getKeysetsCount(): Promise<number> {
        const sql = "SELECT COUNT(*) as count FROM keysets";
    ...
    async getActiveKeysets(): Promise<Keyset[]> {
        const sql =
            "SELECT * FROM keysets WHERE active = 1 ORDER BY mint_url, id";
        const results = await this.database.all<KeysetRow>(sql);
