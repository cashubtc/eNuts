...
export abstract class StoreBase {
	...
	): Promise<SQLResultSetRowList<T>> {
		if (!this._isReady) {
			await this._createStore()
			if (!this._isReady) {
				return getBlankSQLResultSetRowList<T>()
			}
		}
	...
	protected _createStoreSync() {
		...
		this._db.execSync(this.#setupDb, false, (err, resultSet) => {
			if (err) {
				throw err
			}
			if (!resultSet) {
				throw new Error('unable create table: resultSet is null')
			}
	...
	protected async _createStore() {
		...
		try {
			await this._db.exec(this.#setupDb, false)
			this._isReady = true
		} catch (e) {
			l('[_createStore][Error]', e)
		}
	}
	...
	protected async getByKeyPrefix(
		prefix: string,
		{ order = 'ASC', start = 0, count = -1, orderBy = 'insertionOrder' }: ISelectParams = {}
	...
	protected async getObjsByKeyPrefix<T extends object>(
		prefix: string,
		{ order = 'ASC', start = 0, count = -1, orderBy = 'insertionOrder' }: ISelectParams = {}
	): Promise<IKeyValuePair<T>[]> {
		const strKeyValPairs = await this.getByKeyPrefix(prefix, { order, start, count, orderBy })
		if (!strKeyValPairs) {
			return []
		}
		const objKeyValPairs: IKeyValuePair<T>[] = strKeyValPairs.map((x) => ({ key: x.key, value: cTo<T>(x.value) }))
	...
	}: ISelectParams = {}): Promise<IKeyValuePair<string>[]> {
		if (!this._isReady) {
			await this._createStore()
			if (!this._isReady) {
				return []
	...
	protected async getObjsAll<T extends object>({
		order = 'ASC',
		start = 0,
		count = -1,
		orderBy = 'insertionOrder',
	}: ISelectParams = {}): Promise<IKeyValuePair<T>[]> {
	...
	protected async keys({
		order = 'ASC',
		start = 0,
		count = -1,
		orderBy = 'insertionOrder',
	}: ISelectParams = {}): Promise<string[]> {
		if (!this._isReady) {
			await this._createStore()
			if (!this._isReady) {
				return []
	...
	protected async valuesObjs<T extends object>({
		order = 'ASC',
		start = 0,
		count = -1,
		orderBy = 'insertionOrder',
	}: ISelectParams = {}): Promise<T[]> {
		if (!this._isReady) {
			await this._createStore()
			if (!this._isReady) {
				return []
	...
	protected async count(): Promise<number> {
		...
		const result = await this._db.execSelect<{ count: number }>(`select count(*) as count from ${this._name}`)
