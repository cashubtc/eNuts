...
export async function getCustomMintNames<T extends { mintUrl: string }>(
	mints: T[]
): Promise<(T & { customName: string })[]> {
	const data = (await store.getByKeyPrefix(`${MINT_STORE_KEY_PREFIX_NAME}`)).reduce((acc, cur) => {
		acc[cur.key.slice(MINT_STORE_KEY_PREFIX_NAME.length)] = cur.value
		return acc
	}, {} as { [key: string]: string })
	const mintsState: (T & { customName: string })[] = mints.map((m) => ({ ...m, customName: data[m.mintUrl] || '' }))
	return mintsState
}
